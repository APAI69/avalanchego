...
func New(
	...
	m := &manager{
		...
		chainRestarts:   make(map[[32]byte]int),
	}
	m.Initialize()
	return m
}
...
func (m *manager) restartChain(chainParams ChainParameters) {
	chainKey := chainParams.ID.Key()
	if m.chainRestarts == nil {
		m.log.Debug("Aborting chain restart due to node shutting down:\n"+
			"    ID: %s\n"+
			"    VMID:%s",
			chainParams.ID,
			chainParams.VMAlias,
	...
	if numRestarts, exists := m.chainRestarts[chainKey]; exists && numRestarts >= maxChainRestarts {
		m.log.Warn("Aborting chain restart because chain has exceeded maximum restarts:\n"+
			"    ID: %s\n"+
			"    VMID:%s",
			chainParams.ID,
			chainParams.VMAlias,
		...
		m.chainRestarts[chainKey]++
	}
	m.log.Warn("restarting chain:\n"+
		"    ID: %s\n"+
		"    VMID:%s",
		chainParams.ID,
		chainParams.VMAlias,
	...
	chain, err := m.buildChain(chainParams)
	if err != nil {
		m.log.Error("Failed to restart chain with ID: %s", chainParams.ID)
		return
	}

	m.notifyRegistrantsRestart(chain.Ctx, chain.VM)
}
...
func (m *manager) notifyRegistrantsRestart(ctx *snow.Context, vm interface{}) {
	for _, registrant := range m.registrants {
		registrant.RestartChain(ctx, vm)
	}
}
